...
def test(trained_model, test_loader, output_label = None):
    ...
    with torch.no_grad():
        ...
        if output_label is not None:
            ...
            acc = 100.0 * selected_correct / selected_total
            print('The accuracy of the selected classes ', output_label, 'is: ', acc)
        else:
...
def attack(trained_model, attack_model, target_loaders, test_loader):
    ...
    unlearn_X = torch.zeros([1,cf.CLASS_COUNT])
    ...
    test_X = torch.zeros([1,cf.CLASS_COUNT])
    test_X = test_X.cuda()
    
    test_X0 = torch.zeros([1,cf.CLASS_COUNT])
    test_X0 = test_X.cuda()
