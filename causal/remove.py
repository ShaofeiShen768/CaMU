...
def causal_effect_removel(trained_model, new_model, close_loader,sample_loader):
    ...
    optimizer2 = optim.Adam(new_model.parameters(), lr = cf.KL_LR, weight_decay=1e-4) 
    #optimizer3 = optim.Adam(new_model.parameters(), lr = cf.IMPAIR_LR, weight_decay=1e-4) 
    optimizer4 = optim.Adam(new_model.parameters(), lr = cf.IMPAIR_LR, weight_decay=1e-4) 
    ...
    for epoch in range(cf.IMPAIR_EPOCHS):
        ...
        for i, ((data, targets, treatment_x, treatment_y),(remain_x, remain_y)) in enumerate(zip(close_loader,sample_loader)):
            ...
            #embedding4 = 0.5 * embedding4 + 0.5 * embedding3 
